    ...
    <div class="col-lg-4">
        <div class="mb-4">
            ...
            <input type="text" name="price_total" class="form-control @error('price_total') is-invalid @enderror"
                value="{{ optional($bill->billable)->formated_price_total }}"
                max="{{ optional($bill->billable)->price_total }}" readonly>
            @error('price_total')
        ...
        <div class="mb-4">
            ...
            <div class="input-group" id="datepicker1">
                <input type="text" name="bill_date" class="form-control @error('bill_date') is-invalid @enderror"
                    value="{{ $bill->bill_date->format('Y-m-d') }}" data-date-format="yyyy-mm-dd"
        ...
        <div class="mb-4">
            ...
            <select name="bill_mode" class="form-control select2 @error('bill_mode') is-invalid @enderror">
                @foreach ($payments as $payment)
                    <option value="{{ $payment->id }}" {{ $bill->bill_mode === $payment->id ? 'selected' : '' }}>
                        {{ $payment->name }}
                    </option>
                @endforeach
            </select>
...
<div class="docs-options">
    ...
    <div class="input-group mb-4">
        ...
        <input type="text" class="form-control @error('reference') is-invalid @enderror" name="reference"
            value="{{ $bill->reference }}" aria-describedby="ref">
        @error('reference')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
